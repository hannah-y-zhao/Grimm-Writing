"use strict";
//training charRNN model tutorial >> https://github.com/ml5js/training-charRNN
//text corpus from >> https://www.gutenberg.org/ebooks/2591


let pages = 0

let minX, minY, maxX, maxY;
let sq1, sq2, sq3, sq4;

let grimmData
let listWords

let inputName
let inputLines
let inputTitle

let nextButtonTxt = "NEXT"

let welcomeText = "Please enter a name for yourself."
let introTxt = "You will be able to write anything you'd like, and then select from different options generated by the model. Continue these steps until you are satisfied with your story."
let title
let titleCount=0
// text color    not main: fill(200,230,255) main: fill(230, 245, 255)


//firebase
let database
let disclaimer = "Your inputs will be stored in a public database. Please do not enter any sensitive information!"
let ref
let currentName
let fullStory
let firebaseData = []
let fbStoriesArr = []
let fbWordsArr = []
let matchingWordsArr = []
let compareDataArr = []

//charRNN
let charRNN
let tempSeed = "the king said "
let tempVal = 0.5
let charLength = 25
let data = {}

let optionsArr = []
let storyLines
let storyArr = ["Once upon a time"]
let starter = "Once upon a time..."
let counter = 0 //switch from displaying starter to storyArr



function preload() {
  grimmData = loadStrings("grimmStories.txt")
  // console.log(grimmData)
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  cursor(ARROW)

  // textFont("Libre Caslon Text")
  // textFont("Libre Caslon Display")

  grimmData = join(grimmData, " ")
  listWords = splitTokens(grimmData, " ")


  //background visuals
  minX = floor(height / 20);
  minY = floor(height / 20);
  maxX = floor((floor(width - height / 20) - minX) / 5) * 5 + minX;
  maxY = floor((floor(height * 19 / 20) - minY) / 5) * 5 + minY;

  sq1 = {
    x: floor((floor(width * 18 / 20) - minX) / 5) * 5 + minX,
    y: maxY,
    dx: -5,
    dy: 0,
    op: 255
  };
  sq3 = {
    x: sq1.x + 5,
    y: sq1.y,
    dx: -5,
    dy: 0,
    op: 150
  };

  sq2 = {
    x: floor((floor(width * 2 / 20) - minX) / 5) * 5 + minX,
    y: minY,
    dx: 5,
    dy: 0,
    op: 255
  };

  sq4 = {
    x: sq2.x - 5,
    y: sq2.y,
    dx: 5,
    dy: 0,
    op: 150
  };



  inputLines = createInput()
  inputLines.size(width / 2, height / 9)
  inputLines.position(width / 2 - inputLines.width / 2, height / 2 - inputLines.height*3/4)
  inputLines.style("color", "#e6f5ff")
  inputLines.hide()

  inputName = createInput()
  inputName.size(width / 3, height / 12)
  inputName.position(width / 2 - inputName.width / 2, height / 2)
  inputName.style("color", "#e6f5ff")
  inputName.hide()

  inputTitle=createInput()
  inputTitle.size(width/3,height/18)
  inputTitle.position(width/2-inputTitle.width/2,height/7)
  inputTitle.style("color", "#e6f5ff")
  inputTitle.value("Add a title here")
  inputTitle.hide()


  //-----LSTM generator------
  charRNN = ml5.charRNN('grimmModel', modelReady)



  //-----Firebase setup------
  configFirebase()
  database = firebase.database()
  ref = database.ref("stories")
  ref.on("value", gotData)


}

//------Firebase functions------

function configFirebase() {
  const firebaseConfig = {
    apiKey: "AIzaSyAHRmKsixa5X9q0GoATxHBTjT_LoIhws3Y",
    authDomain: "chatbot-final-5b0e8.firebaseapp.com",
    databaseURL: "https://chatbot-final-5b0e8-default-rtdb.firebaseio.com",
    projectId: "chatbot-final-5b0e8",
    storageBucket: "chatbot-final-5b0e8.appspot.com",
    messagingSenderId: "12954650545",
    appId: "1:12954650545:web:9b7087b1acabdfc2435f90",
    measurementId: "G-9C8MNC4XED"
  };
  firebase.initializeApp(firebaseConfig);
}

function toFirebase() {

  fullStory = {
    nickname: currentName,
    storyLines: storyArr
  }
  ref.push(fullStory)
  console.log("fullStory: ", fullStory)


}

function gotData(data) {
  let incomingData = data.val(); // val() returns an object with all the data
  let keys = Object.keys(incomingData); // Object.keys return an array of unique keys

  firebaseData = []
  for (let i = 0; i < keys.length; i++) {
    firebaseData.push(incomingData[keys[i]])
  }
  console.log("firebaseData: ", firebaseData, "keys: ", keys)
  compareFirebase()
}

function compareFirebase() {
  let seq = 0

  fbStoriesArr = []
  fbWordsArr = []
  for (let i = 0; i < firebaseData.length; i++) {
    fbStoriesArr.push(join(firebaseData[i].storyLines, " "))
    console.log(seq++, "name + lines:", firebaseData[i].nickname, firebaseData[i].storyLines)

  }
  for (let i = 0; i < fbStoriesArr.length; i++) {
    fbWordsArr.push(splitTokens(fbStoriesArr[i], " \n"))

  }

 

  console.log("fbWordsArr: ", fbWordsArr, "; fbStoriesArr: ", fbStoriesArr)


  let storyWordsSet
  if (storyArr.length > 1) {
    const joined = join(storyArr, " ")
    const splitting = splitTokens(joined, " ")

    storyWordsSet = new Set(splitting)

    for (let i = 0; i < fbWordsArr.length; i++) {
      for (let j = 0; j < fbWordsArr[i].length; j++) {
        if (storyWordsSet.has(fbWordsArr[i][j])) {
          matchingWordsArr.push([fbWordsArr[i][j], i, j])
          //mWA[0] is word found, mWA[1] is location of story from fbSA
        }
      }
    }
    console.log("mWA: ", matchingWordsArr)
    const pastStoriesCount = [];
    for (let i = 0; i < matchingWordsArr.length; i++) {
      const pastStoryIndex = matchingWordsArr[i][1];
      pastStoriesCount[pastStoryIndex] =
        (pastStoriesCount[pastStoryIndex] || 0) + 1;
    }
    console.log("pastStoriesCount: ", pastStoriesCount);
    const pastStoriesCountWithIndex = pastStoriesCount.map((count, storyId) => {
      return { count, storyId };
    })
    console.log("pastStoriesCountWithIndex orig: ", pastStoriesCountWithIndex);
    pastStoriesCountWithIndex.sort((a, b) => b.count - a.count);
    console.log("pastStoriesCountWithIndex sorted: ", pastStoriesCountWithIndex);
    compareDataArr=[]
    if (storyArr.length > 1) {
      const joined = join(storyArr, " ")
      const splittingArr = splitTokens(joined, " ")
      for (let i = 1; i < 4; i++) {
        console.log("percentage: ", pastStoriesCountWithIndex[i].count / splittingArr.length)
        console.log("storyId: ", pastStoriesCountWithIndex[i].storyId)
        console.log("nickname+ID: ", firebaseData[pastStoriesCountWithIndex[i].storyId].nickname, fbStoriesArr[pastStoriesCountWithIndex[i].storyId])
        compareDataArr.push([pastStoriesCountWithIndex[i].count / splittingArr.length, pastStoriesCountWithIndex[i].storyId, firebaseData[pastStoriesCountWithIndex[i].storyId].nickname])
      }
      console.log(compareDataArr)
    }

  }



}


//------charRNN functions------

function modelReady() {
  console.log("model ready")
}

function generate() {
  data = {
    seed: tempSeed,
    temperature: tempVal,
    length: charLength
  }
  charRNN.generate(data, charGotData)
}
function generate2() {
  data = {
    seed: tempSeed,
    temperature: tempVal,
    length: charLength
  }
  charRNN.generate(data, charGotData2)
}
function generate3() {
  data = {
    seed: tempSeed,
    temperature: tempVal,
    length: charLength
  }
  charRNN.generate(data, charGotData3)
}

function charGotData(err, result) {
  if (err) {
    console.error(err);
    return;
  }

  //------filtering out any extra nonword characters from beginning and end------
  const botGen = result.sample
  let tempBotArr = splitTokens(botGen, " ")

  const listSet = new Set(listWords)
  if (!listSet.has(tempBotArr[0])) {
    tempBotArr.shift()
  }
  if (!listSet.has(tempBotArr[tempBotArr.length - 1])) {
    tempBotArr.pop()
  }

  const filteredBotGen = join(tempBotArr, " ")
  optionsArr.push(filteredBotGen)

  //move onto generating again
  generate2()
}

function charGotData2(err, result) {
  if (err) {
    console.error(err);
    return;
  }

  //------filtering out any extra nonword characters------
  const botGen = result.sample
  let tempBotArr = splitTokens(botGen, " ")

  const listSet = new Set(listWords)
  if (!listSet.has(tempBotArr[0])) {
    tempBotArr.shift()
  }
  if (!listSet.has(tempBotArr[tempBotArr.length - 1])) {
    tempBotArr.pop()
  }

  const filteredBotGen = join(tempBotArr, " ")
  optionsArr.push(filteredBotGen)

  generate3()
}
function charGotData3(err, result) {
  if (err) {
    console.error(err);
    return;
  }

  //------filtering out any extra nonword characters------
  const botGen = result.sample
  let tempBotArr = splitTokens(botGen, " ")

  const listSet = new Set(listWords)
  if (!listSet.has(tempBotArr[0])) {
    tempBotArr.shift()
  }
  if (!listSet.has(tempBotArr[tempBotArr.length - 1])) {
    tempBotArr.pop()
  }

  const filteredBotGen = join(tempBotArr, " ")
  optionsArr.push(filteredBotGen)

}


//------general functions------

function switchPages() {
  if (pages == 4) {
    resetAll()
    pages = 0
    background(103, 128, 151)

  } else {
    pages++
    background(103, 128, 151)
  }
}

function mousePressed() {

  //if within NEXT button
  if (mouseX > width * 20 / 23 - height / 8 && mouseX < width * 20 / 23 + height / 8 && mouseY > height * 10 / 12 - height / 18 && mouseY < height * 10 / 12 + height / 18) {

    if (pages == 2 && inputName.value()) {
      currentName = inputName.value()
      welcomeText = "Hello " + currentName.toString() + ". Please click NEXT to continue writing a story."

      inputName.value("")
      titleCount=0
    }

    if (pages == 3) {
      if (inputLines.value()) {
        storyArr.push(inputLines.value()) //if users press NEXT before ENTER, still stores their input
      }
      toFirebase() //once users move off this page, send data to Firebase
    }
    switchPages()

  }
  if (pages == 3) {
    for (let i = 0; i < 3; i++) {

      //if within any of the options buttons
      if (
        mouseX > width / 7 + (i * width) / 4 &&
        mouseX < width / 7 + (i * width) / 4 + width / 5 &&
        mouseY > (height * 4) / 7 &&
        mouseY < (height * 4) / 7 + (height * 1) / 8 &&
        optionsArr.length > 0
      ) {
        storyArr.push(optionsArr[i])
        optionsArr = []
      }
    }
  }
  if (pages == 4) {
    nextButtonTxt = "RESTART"
  }
}

function keyPressed() {
  if (keyCode === ENTER && pages == 2 && inputName.value()) {
    currentName = inputName.value()
    welcomeText = "Hello " + currentName.toString() + ". Please click NEXT to continue writing a story."

    inputName.value("")

  } else if (keyCode === ENTER && pages == 3 && inputLines.value()) {
    counter = 1
    tempSeed = inputLines.value() //for charRNN generation
    storyArr.push(inputLines.value())

    generate()

    inputLines.value("")
  } else if (keyCode === ENTER && pages == 4 && inputTitle.value()){
    title=inputTitle.value()
    inputTitle.hide()
    titleCount=1
  }
}

//---resets everything after finishing the experience---
function resetAll() {
  welcomeText = "Please enter a name for yourself."
  storyArr = ["Once upon a time"]
  optionsArr = []
  counter = 0
  cursor(ARROW)
  nextButtonTxt = "NEXT"
}



function draw() {
  switch (pages) {
    case 0: page0intro()
      break;
    case 1: page1overview()
      break;
    case 2: page2name()
      break;
    case 3: page3main()
      break;
    case 4: page4view()
      break;
  }
}

function page0intro() {

  allBackground()
  nextButton()

  fill(230, 245, 255)
  noStroke()
  textAlign(CENTER)

  textFont("Cormorant Garamond")
  textSize(70)
  text("A Grimm Creativity", width / 2, height * 1 / 2)
  textFont("Libre Caslon Text")
  textSize(18)
  text("What does it mean to be unique?", width/2,height*25/44)

}

function page1overview() {
  allBackground()
  nextButton()

  fill(230, 245, 255)
  noStroke()
  textAlign(CENTER)
  textFont("IBM Plex Mono")
  // textFont("Libre Caslon Text")
  textSize(22)
  text("\nThis is a collaborative writing experience between you and a machine learning model trained on the Grimms' Fairy Tales.", width * 1 / 2, height * 3 / 9, width * 7 / 10, height * 1 / 7)
  textSize(30)
  text("\n"+introTxt,width * 1 / 2, height * 6 / 10, width * 4 / 5, height * 1 / 3)
}

function page2name() {
  allBackground()
  nextButton()

  inputName.show()
  textAlign(CENTER)
  fill(230, 245, 255)
  noStroke()
  textSize(30)
  textFont("IBM Plex Mono")
  text(welcomeText + "\nPress ENTER to record your name, and the NEXT button to move on", width / 2, height * 2 / 5)

  textSize(20)
  text(disclaimer, width / 2, height * 3 / 4 + inputName.height * 2, width * 2 / 3, height / 2)

}

function page3main() {
  allBackground()

  inputName.hide()
  inputLines.show()

  let opacity2 = 10
  let value = color(0, 15, 30)

  fill(230, 245, 255)
  noStroke()
  textSize(20)
  text("Press ENTER after writing to record your story.", width / 2, height * 2 / 6, width / 2, height / 3)

  cursor(ARROW)

  textAlign(CENTER)
  if (counter == 0) {
    text(starter, width / 2, height * 1 / 3)
  } else if (counter == 1) {
    allBackground()
    noStroke()

    nextButton()
    textSize(20)
    fill(230, 245, 255)

    text("Select a machine generated option below, or NEXT to move on.", width / 2, height * 2 / 6, width / 2, height / 3)

    textSize(20)
    let tempJoined = join(storyArr, " ")
    text(tempJoined, width / 2, height * 5 / 12, width * 4 / 5, height * 1 / 4)
  }
  for (let i = 0; i < 3; i++) {
    if (optionsArr.length > 0) {
      rectMode(CORNER)
      if (
        mouseX > width / 7 + (i * width) / 4 &&
        mouseX < width / 7 + (i * width) / 4 + width / 5 &&
        mouseY > (height * 4) / 7 &&
        mouseY < (height * 4) / 7 + (height * 1) / 8
      ) {

        cursor(HAND);
        value = 0, 15, 30;
        opacity2 = 30;
      } else {
        value = 255;
        opacity2 = 0;

      }
      stroke(230, 245, 255, 100)
      rectMode(CORNER)
      fill(value, opacity2);
      rect(
        width / 7 + (i * width) / 4,
        (height * 4) / 7,
        width / 5,
        (height * 1) / 8
      );
      textSize(15)
      noStroke()
      fill(230, 245, 255)
      textAlign(CENTER)
      fill(230, 245, 255)
      text(optionsArr[i], width / 7 + (i * width) / 4, (height * 5) / 8, width / 5, (height * 1) / 8)
    } else {
      stroke(230, 245, 255, 100)
      rectMode(CORNER)
      fill(value, 30);
      rect(
        width / 7 + (i * width) / 4,
        (height * 4) / 7,
        width / 5,
        (height * 1) / 8
      );
    }
  }
}

function page4view() {
  allBackground()
  nextButton()

  inputLines.hide()

  textAlign(CENTER)
  const joined = join(storyArr, " ")

  if (titleCount==0){
    inputTitle.show()
  }else if (titleCount==1){
    textFont("Cormorant Garamond")
    fill(230, 245, 255)
    textSize(50)
    inputTitle.hide()
    text(title,width/2,height*3/11)
  }

  textFont("Libre Caslon Text")
  fill(230, 245, 255)
  textSize(20)
  text("\n\nby the Brothers Grimm, charRNN, and " + currentName, width / 2, height / 2, width * 4 / 5, height * 5 / 9)
  textSize(25)
  text("\n\n"+joined, width / 2, height *7/13, width * 4 / 5, height * 5 / 9)

  noStroke()
  fill(0,20)
  rect(width/2,height*3/5,width*15/20,height*4/14-20)

  textFont("IBM Plex Mono")
  fill(230, 245, 255)
  text("Your story is:", width/2, height*5/9)
  
  for (let i=0; i<compareDataArr.length;i++){
    
    text("\n"+Math.round(compareDataArr[i][0]*100)+"% similar to\n"+compareDataArr[i][2]+"'s story",width*2/7-height/20+i*width/4,height*3/5)
  }
  

}


function allBackground() {
  background(103, 128, 151)

  rectMode(CORNER)
  noFill()
  strokeWeight(7)
  stroke(0, 255)
  rect(minX, minY, floor(width - height * 2 / 20), floor((floor(height * 19 / 20) - minY) / 5) * 5)

  // stroke(120, 175, 230, 95)
  stroke(140,220,255,95)
  rect(minX, minY, floor(width - height * 2 / 20), floor((floor(height * 19 / 20) - minY) / 5) * 5)

  rectMode(CENTER)
  moveSq(sq1);
  drawSq(sq1);

  moveSq(sq3);
  drawSq(sq3);

  moveSq(sq2);
  drawSq(sq2);

  moveSq(sq4);
  drawSq(sq4);
}

function moveSq(sq) {

  rectMode(CENTER)
  if (sq.dx < 0 && sq.x <= minX) {
    // Hitting left wall
    // Switch to going up
    sq.dx = 0;
    sq.dy = -5;
  } else if (sq.dy < 0 && sq.y <= minY) {
    // Hitting the top wall
    // Switch to going right
    sq.dx = 5;
    sq.dy = 0;
  } else if (sq.dx > 0 && sq.x >= maxX) {
    // Hitting the right wall
    // Switch to going down
    sq.dx = 0;
    sq.dy = 5;
  } else if (sq.dy > 0 && sq.y >= maxY) {
    // Hitting the bottom wall
    // Switch to going left
    sq.dx = -5;
    sq.dy = 0;
  }

  sq.x += sq.dx;
  sq.y += sq.dy;
}

function drawSq(sq) {
  rectMode(CENTER)

  noStroke()
  fill(200, 230, 255, sq.op);
  square(sq.x, sq.y, 7);
}

function nextButton() {
  rectMode(CENTER)
  let opacity = 0
  if (mouseX > width * 20 / 23 - height / 8 && mouseX < width * 20 / 23 + height / 8 && mouseY > height * 10 / 12 - height / 18 && mouseY < height * 10 / 12 + height / 18) {
    opacity = 40
    cursor(HAND)
  } else {
    opacity = 0
    cursor(ARROW)
  }


  noStroke()
  fill(0, 15, 30, opacity)
  rect(width * 20 / 23, height * 10 / 12, height / 4, height / 9)

  stroke(200, 230, 255)
  strokeWeight(4)
  noFill()
  rect(width * 20 / 23, height * 10 / 12, height / 4 + 5, height / 9 + 5)

  textFont("IBM Plex Mono")
  noStroke()
  fill(200, 230, 255)
  textSize(28)
  text(nextButtonTxt, width * 20 / 23, height * 85 / 100)
}

